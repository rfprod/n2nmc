...
if (process.env.OPENSHIFT_MONGODB_DB_HOST) {
	...
	app.use(session({secret:'secretNG2NODEMONGO', resave:false, saveUninitialized:true, store: store , cookie: {
		maxAge: 1000 * 60 * 60 * 24 * 1 // 1 day 
	} }));
}else{
	app.use(session({secret:'secretNG2NODEMONGO', resave:false, saveUninitialized:true}));
}
